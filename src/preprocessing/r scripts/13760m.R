...
data <- getGEO("GSE13760", GSEMatrix=TRUE)

pData(data[[1]])

t<-endsWith(pData(data[[1]])$title, "case")
...
#write.csv(mdata, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/pGSE13760m.csv")
write.table(expr, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE13760m.csv", col.names = NA)


###


data <- read.csv(file='~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE13760m.csv',sep="",header=TRUE)
...
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE13760m.csv", sep=",", col.names=NA)
