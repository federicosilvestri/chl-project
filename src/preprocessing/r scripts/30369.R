...
data <- getGEO("GSE30369", GSEMatrix=TRUE)

pData(data[[1]])

p<- endsWith(pData(data[[1]])$characteristics_ch1.1, "affected")
patients <- pData(data[[1]])[p,]

expr <- t(exprs(data[[1]]))
data <- subset(expr, rownames(expr) %in% rownames(patients))
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE30369m.csv", col.names = NA)


###


data <- read.csv(file='~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE30369m.csv',sep="",header=TRUE)
...
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE30369m.csv", sep=",", col.names=NA)
