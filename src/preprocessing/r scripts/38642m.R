...
data <- getGEO("GSE38642", GSEMatrix=TRUE)

t<-startsWith(pData(data[[1]])$characteristics_ch1.5, "status: T2D donors")
...
write.csv(mdata, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/pGSE38642m.csv")
write.table(expr, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/eGSE38642m.csv", col.names = NA)


###

data <- read.csv(file='~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE38642m.csv',sep="",header=TRUE)
...
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE38642m.csv", sep=",", col.names=NA)
