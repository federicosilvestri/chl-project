...
data <- getGEO("GSE162622", GSEMatrix=TRUE)

pData(data[[1]])

t<-endsWith(pData(data[[1]])$title, "pre VSL3 supplements plasma")
...
write.csv(patients, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/pGSE162622m.csv")
write.table(expr, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/eGSE162622m.csv", col.names=NA)


###


data <- read.csv(file='~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE162622m.csv',sep="",header=TRUE)
...
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE162622m.csv", sep=",", col.names=NA)
