...
data <- getGEO("GSE54350", GSEMatrix=TRUE)

p<-startsWith(pData(data[[1]])$characteristics_ch1.1, "group: Diabetic")
...
#write.csv(patients, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/pGSE54350m.csv")
write.table(expr, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/eGSE54350m.csv", col.names=NA)

###

data <- read.csv(file='~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE54350m.csv',sep="",header=TRUE)
...
# Get gene symbol
x <- as.list(illuminaHumanv4SYMBOL)
...
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE54350m.csv", sep=",", col.names=NA)
