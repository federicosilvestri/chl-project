...
data <- getGEO("GSE26887", GSEMatrix=TRUE)

pData(data[[1]])

t<-startsWith(pData(data[[1]])$title, "heart_diabetic")
...
write.csv(mdata, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/pGSE26887m.csv")
write.table(expr, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/eGSE26887m.csv", col.names = NA)


###


data <- read.csv(file='~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE26887m.csv',sep="",header=TRUE)
...
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE26887m.csv", sep=",", col.names=NA)
