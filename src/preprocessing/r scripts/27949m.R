...
data <- getGEO("GSE27949", GSEMatrix=TRUE)

pData(data[[1]])

t<-!endsWith(pData(data[[1]])$characteristics_ch1, "NGT")
...
#write.csv(mdata, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/pGSE27949m.csv")
write.table(expr, "~/Desktop/Pisa/y2s2/chl/prj/mdatasets/eGSE27949m.csv", col.names = NA)


###


data <- read.csv(file='~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE27949m.csv',sep="",header=TRUE)
...
write.table(data, "~/Desktop/Pisa/y2s2/chl/prj/chl-prj/dataset/eGSE27949m.csv", sep=",", col.names=NA)
